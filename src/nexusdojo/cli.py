...
# Location of static, in-package main.py templates for kata scaffolds.
STATIC_TEMPLATES_ROOT = Path(__file__).resolve().parent / "templates"
...
# ═══════════════════════════════════════════════════════════════════════════════
# KEYBOARD INPUT HELPERS (for arrow key navigation)
# ═══════════════════════════════════════════════════════════════════════════════

def read_key() -> str:
    """
    Read a single keypress. Returns special strings for arrow keys.
    Works on Unix/macOS with termios/tty.
    """
    fd = sys.stdin.fileno()
...
def handle_watch(args: argparse.Namespace) -> int:
    """
    Watch for file changes and auto-run dojo check (Polling Mode - Optimized for Speed).
    """
    import time
    ...
    console.print(f"[bold cyan]Dojo Watch (Turbo Mode)[/bold cyan]")
    console.print(f"Watching {project_dir} for .py changes...")
    console.print("Press Ctrl+C to stop.")

    def get_mtimes(directory: Path) -> dict[Path, float]:
        """Return a dict of file paths and their modification times."""
        mtimes = {}
        try:
    ...
    # Initial scan
    last_mtimes = get_mtimes(project_dir)

    # Initial run
    args.concise_mode = True
    console.clear()
    handle_check(args)
    ...
    try:
        while True:
            time.sleep(0.5)
            current_mtimes = get_mtimes(project_dir)
            ...
            if changed:
                ...
                # Run the check
                # We reuse handle_check logic but ensure it prints directly
                try:
                    args.concise_mode = True
                    handle_check(args)
...
def handle_check(args: argparse.Namespace) -> int:
    ...
    # Auto-install deps - DISABLED FOR SPEED (User can pip install manually if needed)
    ...
    # Run unittest (explicit discovery keeps kata structure consistent)
    ...
    concise = getattr(args, "concise_mode", False)
    ...
    if result.returncode == 0:
        ...
        if concise:
            # Concise Mode: Print summary only
            lines = error_out.splitlines()
            summary = []
            capturing = False
            for line in lines:
                # Start capturing at the first failure/error header or traceback
                if line.startswith(("FAIL:", "ERROR:", "Traceback")):
                    capturing = True
                if capturing:
                    summary.append(line)
            
            # If we didn't find a standard header, just show the last 15 lines
            if not summary:
                summary = lines[-15:]
            
            # Limit to 20 lines max to fit in pane
            display_text = "\n".join(summary[:20])
            console.print(Panel(
                display_text,
                title="[bold red]❌ FAILURE SUMMARY[/bold red]",
                border_style="red"
            ))
            ...
            # AI Diagnosis - DISABLED FOR SPEED
...
def build_mission_header_comments(project_dir: Path, max_lines: int = 120, max_chars: int = 4000) -> list[str]:
    """
    Read MISSION.md and return a concise commented block for in-file reference.
    """
    mission_path = project_dir / "MISSION.md"
    if not mission_path.exists():
        return []
    try:
        raw_lines = mission_path.read_text(encoding="utf-8", errors="ignore").splitlines()
    except Exception:
        return []

    header = ["# --- MISSION.md (synced for quick reference) ---"]
    total = 0
    for line in raw_lines:
        if len(header) > max_lines or total + len(line) > max_chars:
            header.append("# ... (see MISSION.md for full brief)")
            break
...
def handle_start(args: argparse.Namespace) -> int:
    ...
    # Always drop in the static, annotated main.py for this template.
    static_main = STATIC_TEMPLATES_ROOT / resolved_template / "main.py"
    if static_main.exists():
        shutil.copy(static_main, target_dir / "main.py")
    else:
        console.print(f"[yellow]Warning:[/yellow] Static main.py not found for template '{resolved_template}'. Using packaged template version.")
    ...
    mission_header_lines = build_mission_header_comments(target_dir)

    # Prepend mission to existing static main.py
    main_py = target_dir / "main.py"
    if mission_header_lines and main_py.exists():
        original_content = main_py.read_text()
        already_injected = original_content.lstrip().startswith("# --- MISSION.md")
        if not already_injected:
            main_py.write_text("\n".join(mission_header_lines) + "\n\n" + original_content.lstrip("\n"))
...
def handle_continue(args: argparse.Namespace) -> int:
    ...
    if interactive and project_dir.exists():
        console.print("[dim]Launching editor + watch...[/dim]")
...
def print_splash():
    ...
    console.print(Align.center("[dim]Initializing System...[/dim]"))
    time.sleep(1.0) # The "Premium" pause
def handle_menu(args: argparse.Namespace) -> int:
    """
    Interactive menu for common tasks (Rich UI - Dashboard).
    """
    print_splash() # Splash

    # Normalize args with defaults so downstream handlers have the expected attributes.
    if not hasattr(args, "root"):
        args.root = str(DEFAULT_KATA_ROOT)
    if not hasattr(args, "notes_root"):
        args.notes_root = str(DEFAULT_NOTES_ROOT)

    kata_root = Path(getattr(args, "notes_root", DEFAULT_KATA_ROOT)) # Fallback if args.root missing
    # Actually, args.root is usually kata root. notes_root is notes.
    # Let fix the args access properly.
    kata_root = Path(getattr(args, "root", DEFAULT_KATA_ROOT))
    notes_root = Path(getattr(args, "notes_root", DEFAULT_NOTES_ROOT))
    ...
    # run_boot_diagnostics(kata_root, notes_root) # Disabled to avoid potential crash if missing

    # Require an active profile before continuing
    ensure_profile_selected(notes_root)

    # Track if entrance animation has played this session
    ...
    while True:
        ...
        try:
            ...
            # ═══ ENTRANCE ANIMATION (optional, <300ms) ═══
            settings_check = load_settings(notes_root)
            show_animation = settings_check.get("menu_animation", True)

            if show_animation and not entrance_played:
                # Subtle line-by-line fade-in effect
                console.print()
                time.sleep(0.05)
            ...
            # active_kata_dir = find_active_kata_dir(Path.cwd()) # This might not exist in your version yet
            # Let assume we are in lobby unless specified
            active_kata_dir = None
            if (Path.cwd() / "MISSION.md").exists():
                 active_kata_dir = Path.cwd()
            ...
            completed_drills = count_completed_drills(kata_root, notes_root)
            # heatmap = get_streak_heatmap(notes_root, days=7) # Might fail if missing helper
            heatmap = "[dim]Streak tracking...[/dim]" 
            ...
            resume_slug = get_last_session(kata_root)

            # ═══ MENU ITEMS DEFINITION ═══
            menu_items = [
                {"key": "1", "label": "The Gym", "desc": "drills & practice", "action": "gym"},
                {"key": "2", "label": "The Workshop", "desc": "build projects", "action": "workshop"},
                {"key": "3", "label": "Boss Battle", "desc": "test your skills", "action": "boss"},
            ]
            if resume_slug:
                menu_items.append({"key": "4", "label": f"Resume: {resume_slug}", "desc": "", "action": "resume"})

            # Divider index (after core actions)
            divider_after = len(menu_items)

            menu_items.extend([
                {"key": "5", "label": "Profile", "desc": "", "action": "profile"},
                {"key": "6", "label": "Settings", "desc": "", "action": "settings"},
                {"key": "7", "label": "Help", "desc": "", "action": "help"},
                {"key": "8", "label": "Exit", "desc": "", "action": "exit"},
            ])

            # ═══ CONTEXT BADGE COLORS ═══
            def get_context_style(kata_dir):
                """Return badge style based on context."""
                if kata_dir:
                    return "black on cyan", kata_dir.name[:18]
                return "white on grey30", "LOBBY"

            badge_style, badge_text = get_context_style(active_kata_dir)
            prompt_color = "cyan" if active_kata_dir else "grey50"

            # ═══ BUILD MENU RENDERABLE ═══
            def get_skill_state(xp: int) -> str:
                if xp >= 50:
                    return "practiced"
                elif xp > 0:
                    return "touched"
            ...
            # Mantra (fixed for session to avoid flicker on minute change)
            from datetime import datetime as dt
            mantras = [
                "Train deliberately.",
                "Small reps. Real mastery.",
                "Show up. Ship code.",
                "Consistency over intensity.",
                "One kata at a time.",
                "The path is the practice.",
            ]
            session_mantra = mantras[dt.now().minute % len(mantras)]

            def build_menu(selected_idx: int) -> Group:
                """Two-column HEADER (status | memory) + single-column MENU below, stretched."""

                # ═══ CONTENT WIDTH ═══
                terminal_width = console.width
                content_width = max(60, terminal_width - 4)
                ...
                skills_table = Table.grid(padding=(0, 2))
                skills_table.add_column(width=18)
                skills_table.add_column(justify="right")

                for pillar, xp in skills.items():
                    if pillar == "mixed":
                        continue
                    state_label = get_skill_state(xp)
                    skills_table.add_row(
                        Text.from_markup(f"  [dim]{format_pillar_label(pillar)}[/dim]"),
                        Text.from_markup(f"[dim]{state_label}[/dim]"),
                    )

                h_left.add_row(skills_table)

                # ═══ HEADER RIGHT: Last Activity ═══
                h_right = Table.grid(padding=0)
                h_right.add_column()
                ...
                if recent_entries:
                    ...
                    proj, ts, note = last
                    date_str = ts.split(' ')[0]
                    kata_name = proj.replace('-', ' ')
                    h_right.add_row(Text.from_markup("[dim]Last session:[/dim]"))
                    h_right.add_row(Text.from_markup(f"[white]{kata_name}[/white]"))
                    if note and note.strip():
                        h_right.add_row(Text.from_markup(f"[dim]→ {summarize_text(note, 28)}[/dim]"))
                    h_right.add_row("")
                ...
                # ═══ MENU (single column) ═══
                menu_grid = Table.grid(padding=0)
                menu_grid.add_column(width=content_width)

                menu_label_width = max(len(item["label"]) for item in menu_items) + 2

                for i, item in enumerate(menu_items):
                    if i == divider_after:
                        menu_grid.add_row("")
                    is_selected = (i == selected_idx)
                    label = item["label"].ljust(menu_label_width)
                    desc = item["desc"]
                    if is_selected:
                        desc_markup = f"[dim]{desc}[/dim]" if desc else ""
                        row = f"[cyan]›[/cyan] [bold white]{label}[/bold white]"
                        if desc_markup:
                            row += f"  {desc_markup}"
                    else:
                        if desc:
                            row = f"  [dim]{label}  {desc}[/dim]"
                        else:
                            row = f"  [dim]{label}[/dim]"
                    menu_grid.add_row(Text.from_markup(row))

                # ═══ CONTENT CONTAINER (centered) ═══
                content = Table.grid(padding=0)
                content.add_column(width=content_width)
                content.add_row(header)
                content.add_row("")
                content.add_row(Align.center(Text.from_markup(f"[italic]{session_mantra}[/italic]"), width=content_width))
                content.add_row("")
                content.add_row(Text.from_markup(f"[grey30]{'-' * content_width}[/grey30]"))
                content.add_row("")
                content.add_row(menu_grid)
                content.add_row("")
                content.add_row(Align.center(Text.from_markup("[grey37]↑↓ select · enter confirm · q quit[/grey37]"), width=content_width))

                # ═══ ASSEMBLE ═══
                lines = []
                lines.append("")
                lines.append(Align.center("[bold white]N E X U S   D O J O[/bold white]"))
                lines.append("")
                lines.append(Align.left(content))
            ...
            def render_menu(selected_idx: int):
                ...
                console.print(build_menu(selected_idx))
            ...
            while True:
                ...
                if key == 'UP':
                    ...
                    choice = menu_items[selected_idx]["key"]
                    break
                    ...
                    for i, item in enumerate(menu_items):
                        ...
                        continue
                    break
                elif key == 'q' or key == 'Q' or key == 'EXIT':
                    choice = "8"
                    break
                elif key == 'ESC':
                    continue
            ...
            if choice == "1":
                def arrow_select(title: str, options: list[str]) -> int:
                    sel = 0
                    console.show_cursor(False)
                    while True:
                        console.clear()
                        ...
                        for i, opt in enumerate(options):
                            if i == sel:
                                console.print(Text.from_markup(f"[cyan]›[/cyan] [bold white]{opt}[/bold white]"))
                            else:
                                console.print(Text.from_markup(f"  [dim]{opt}[/dim]"))
                        console.print("\n[grey37]↑↓ select · enter confirm · q back[/grey37]")
                        key = read_key()
                        if key == "UP":
                ...
                console.print("[yellow]Workshop under construction.[/yellow]")
                time.sleep(1)
            elif choice == "3":
                console.print("[red]Not ready for Boss Battle.[/red]")
                time.sleep(1)
            elif choice == "4":
...
def apply_kata_scaffold(
    ...
    if template == "script":
        main_lines: list[str] = []
        if mission_header_lines:
            main_lines.extend(mission_header_lines)
            main_lines.append("")
        main_lines.extend(
            [
        ...
        for fn in functions:
            ...
            # Add a gentle hint for beginners
            hint = ""
            ...
            if name.startswith("add"):
                ...
                hint = "return a / b  # handle b == 0 if you want"
            if hint:
                main_lines.append(f"    # hint: {hint}")
            else:
                main_lines.append("    # TODO: replace this with your solution")
        ...
        for route in routes:
            ...
            if desc:
                main_lines.append(f"    \"\"\"{desc}\"\"\"")
            main_lines.append("    # TODO: return a JSON-friendly dict with your result")
            main_lines.append("    return {\"message\": \"Hello from your API\"}")
            main_lines.append("")
        main_lines.append("")
    ...
    if not smoke_path.exists():
        if template == "fastapi":
            smoke_content = (
                ...
                "    def test_app_exposes_fastapi_instance(self) -> None:\n"
...
def main(argv: Optional[Iterable[str]] = None) -> int:
    ...
    try:
        if func is None:
            # No command provided: show login page, then go to menu
            notes_root = Path(DEFAULT_NOTES_ROOT)
            show_login_page(notes_root)
            return handle_menu(argparse.Namespace())
